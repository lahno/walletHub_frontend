    ...
    <li class="flex justify-between gap-x-6 py-2" v-for="wallet in wallets" :key="wallet.id">
      ...
      <div class="flex min-w-0 gap-x-4 mt-1">
        <div class="min-w-0 flex-auto">
          <div :class="[
                  'group relative flex items-center justify-between border-gray-500 rounded px-3 py-2 bg-slate-50',
                  isLoading && 'bg-slate-100',
                ]"
          >
            <div class="size-6 absolute left-50" v-if="isLoading">
              <svg
                  aria-hidden="true"
                  role="status"
                  class="inline w-4 h-4 animate-spin text-red-300 mr-2"
                  viewBox="0 0 100 101"
                  fill="none"
            ...
            <span :class="[
                  'font-mono text-sm text-slate-700 select-all',
      ...
      <div class="flex min-w-0 gap-x-1">
        <AddressDropdownMenu :wallet="wallet" @update-wallet-list="$emit('update-wallet-list')" />
      </div>
    </li>
  </ul>
</template>