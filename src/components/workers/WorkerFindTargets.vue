...
export default {
  ...
  methods: {
      ...
      this.socket = initWebSocket(
          ...
          (task_data) => {
            ...
            if (task_data.progress === 100) {
              ...
              // Обновить список таргетов в компоненте TargetsView.vue метод fetchTargets
    ...
    async stopTask() {
      ...
      const response = await axios.get(`/websocket/stop-task/${this.task.task_id}/`);
      if (response.status === 200 && response.data.status === true) {
        toastStore.showToast("Процесс прерван", true);
        this.$emit("update:btn", this.task.name);
        this.stopped = true;
        ...
        toastStore.showToast("Ошибка прекращения процесса!", false, 15000);
      }
    },
    closeWebSocket() {
  ...
  mounted() {
    ...
    console.log("Компонент WorkerFindTargets смонтирован с task:", this.task);
  ...
  beforeUnmount() {
    ...
    console.log("Компонент WorkerFindTargets размонтирован:", this.task?.task_id);
  },
};
</script>
